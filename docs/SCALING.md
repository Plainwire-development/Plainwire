...
Plainwire 2.1.0 is designed to stay simple on a small self-hosted instance while keeping the hot paths bounded enough to grow. This document describes the actual 2.1.0 architecture, what scales horizontally today, which limits are intentional, and how to test a host before raising them.
...
The built-in RESP client does **not** implement Redis Cluster `MOVED`/`ASK` redirect discovery in 2.1.0. Point it at a normal single Redis endpoint (including a managed/HA endpoint or compatible proxy that hides topology) rather than directly at a native Redis Cluster shard endpoint. See `docs/REDIS.md`.
...
TURN and an SFU solve different problems. Cloudflare TURN gives Plainwire a globally operated relay path when peers cannot connect directly, but it does not change Plainwire 2.1.0's full-mesh WebRTC topology.

For a room of `N` participants, a full mesh has `N * (N - 1) / 2` peer relationships and each sender may upload to `N - 1` peers. The default participant/share limits exist to protect client CPU/uplink and are not merely server limits. Large voice/video rooms or thousands of users simultaneously participating in large calls should move to an SFU architecture. `pw_media_topology` is the explicit boundary for that future implementation; 2.1.0 reports `mesh` and does not fake an SFU backend.
...
The last operation necessarily ends current 2.1.0 realtime sessions/calls because automatic realtime-owner failover is not implemented. Document that limitation in production operations rather than pretending otherwise.
