...
The refreshed call panel shows the native score and a recent-jitter chart, while avoiding DOM work when closed. For a browser-to-native display check, build the native worker and run `PLAINWIRE_TEST_NATIVE=1 npm run test:rtc`.
