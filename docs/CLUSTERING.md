...
Use `pw_cluster:status()` to inspect readiness, membership, queue depth and sent/received/drop counters. It contains no credentials. Losing Partisan does not crash local chat/calls. Losing the single realtime owner ends its current realtime sessions/calls; clients must reconnect. Automatic owner failover is not implemented in 2.3.0.
