...
`make` alone prints help. Dependencies install through `npm ci` only when the lockfile changes or the dependency stamp is absent. Grouped frontend outputs rebuild together when source changes or an output is removed, including under `make -j`. The native target tracks both source files and its build script. Rebar manages Erlang compilation incrementally. After changing compiler selection, run `make -B native FC=gfortran CC=cc` to force a native rebuild.
