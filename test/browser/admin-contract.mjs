...
assert.match(js,/expected_updated_at:\s*b\.updated_at[\s\S]*expected_updated_at:\s*existing\.updated_at/,'admin pause and edit requests carry the banner revision');
...
assert.match(js,/form\.id\s*=\s*["']account-moderation-form["'][\s\S]*apply\.setAttribute\(\s*["']form["'],\s*form\.id\s*\)/,'ban and suspend footer buttons submit the moderation form rather than sitting inert outside it');
assert.match(js,/showModal[\s\S]*HTMLFormElement[\s\S]*type\s*===\s*["']submit["'][\s\S]*setAttribute\(\s*["']form["'],\s*body\.id\s*\)/,'modal footer submit buttons are associated with a form body so they send the request');
assert.match(js,/confirmAction[\s\S]*go\.type\s*=\s*["']button["'][\s\S]*addEventListener\(\s*["']click["']/,'restore and other confirm-modals use explicit click handlers on footer buttons');
assert.match(js,/function parseLocalDateTime\(value\)\s*\{[\s\S]*if\s*\(\s*!value\s*\)\s*return null[\s\S]*endRaw\s*&&\s*endValue\s*===\s*null[\s\S]*endValue\s*>\s*0\s*&&\s*endValue\s*<=\s*startValue/,'admin banner editor rejects malformed or non-forward schedule windows instead of translating them into permanent banners');
assert.match(db,/<<"disable">>[\s\S]*<<"revoke_sessions">>[\s\S]*<<"clear_display_name">>[\s\S]*<<"remove_email">>[\s\S]*<<"resend_verification">>/,'operator account actions cover disable, session revocation, display name, and email');
