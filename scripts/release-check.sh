...
for command_name in node npm elm erl rebar3; do need "${command_name}"; done
...
grep -Fq 'Plainwire 1.5.0 visual refinement and call workspace' priv/static/app.css
...
grep -Fq 'Plainwire 1.5.0 visual refinement and call workspace' "${release_root}"/lib/plainwire_relay-*/priv/static/app.css
grep -Fq 'data-ui-version' "${release_root}"/lib/plainwire_relay-*/priv/static/app.js

printf 'Plainwire 1.5.0 release checks passed.\n'
