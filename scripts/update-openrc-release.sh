...
grep -Fq 'Plainwire 1.7.2-2 workspace' "${REPO}/priv/static/app.css" || { printf 'New UI CSS fingerprint is missing.\n' >&2; exit 1; }
...
curl -fsS http://127.0.0.1:8080/api/version | grep -Fq '1.7.2-2' || { printf 'Running backend version check failed.\n' >&2; exit 1; }
...
printf 'Plainwire 1.7.2-2 is running with the refreshed frontend.\n'
