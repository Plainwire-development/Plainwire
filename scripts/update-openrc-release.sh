...
grep -Fq 'Plainwire 1.5.0 visual refinement and call workspace' "${REPO}/priv/static/app.css" || { printf 'New UI CSS fingerprint is missing.\n' >&2; exit 1; }
...
curl -fsS http://127.0.0.1:8080/api/version | grep -Fq '1.5.0' || { printf 'Running backend version check failed.\n' >&2; exit 1; }
curl -fsS http://127.0.0.1:8080/assets/app.css | grep -Fq 'Plainwire 1.5.0 visual refinement and call workspace' \
...
printf 'Plainwire 1.5.0 is running with the refreshed frontend.\n'
