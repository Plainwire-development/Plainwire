...
grep -Fq 'Plainwire 1.7.1 workspace' "${REPO}/priv/static/app.css" || die "New UI CSS fingerprint is missing."
...
curl -fsS http://127.0.0.1:8080/api/version | grep -Fq '1.7.1' \
    || die "Plainwire answered, but the running release is not 1.7.1."
